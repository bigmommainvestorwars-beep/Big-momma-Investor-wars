...
export const CircularBoard52: React.FC<CircularBoard52Props> = ({
  ...
  const spaceOwnersMap = useMemo(() => {
    const map = new Map<number, { playerId: string; playerIndex: number; displayName: string }>();
    players.forEach((p, pIdx) => {
      (p.ownedSpaceIds || []).forEach((sId) => {
        const sp = DEFAULT_STANDARD_SPACES.find((s) => s.id === sId);
        if (sp) {
          map.set(sp.index, { playerId: p.id, playerIndex: pIdx, displayName: p.displayName });
        }
      });
    });
    return map;
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="flex-1 w-full flex flex-col items-center justify-center px-0.5 py-0.5 text-center leading-[1.05]">
