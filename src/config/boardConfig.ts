/**
 * Production Configuration Boundary: Board Configuration
 */

import { Board, BoardSpace } from '../types/board';
...
export interface BoardConfigRegistry {
  getBoardConfig(boardId: string): BoardConfig | undefined;
  registerBoardConfig(config: BoardConfig): void;
}

class InMemoryBoardRegistry implements BoardConfigRegistry {
  private boards = new Map<string, BoardConfig>();

  public registerBoardConfig(config: BoardConfig): void {
    this.boards.set(config.boardId, config);
  }

  public getBoardConfig(boardId: string): BoardConfig | undefined {
    return this.boards.get(boardId);
  }
}

export const DEFAULT_STANDARD_SPACES: BoardSpace[] = [
...
export const defaultStandardBoard: BoardConfig = {
  boardId: 'default-standard-board',
  name: "BIG MOMMA: INVESTORS' WAR Financial District",
  version: '1.0.0',
  totalSpaces: TOTAL_BOARD_SPACES,
  spaces: DEFAULT_STANDARD_SPACES,
};

export const boardRegistry = new InMemoryBoardRegistry();
boardRegistry.registerBoardConfig(defaultStandardBoard);
