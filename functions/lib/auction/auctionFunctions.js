...
exports.resolveAuction = exports.passAuction = exports.placeBid = exports.createAuction = void 0;
 ...
 * 11. createAuction
 */
exports.createAuction = (0, https_1.onCall)(async (request) => {
...
exports.placeBid = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    rateLimiter_1.RateLimiter.check(auth.userId, 'placeBid');
    const { matchId, requestId, payload } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const auctionRef = matchRef.collection('auctions').doc(payload.auctionId);
    const bidderId = payload.actingPlayerId || auth.userId;
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, bidderId, 'placeBid', async (t) => {
        const auctionSnap = await t.get(auctionRef);
        if (!auctionSnap.exists) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUCTION_NOT_FOUND, 'Auction not found.');
        }
        const auction = auctionSnap.data();
        if (auction.status !== 'active') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUCTION_NOT_ACTIVE, 'Auction is no longer active.');
        }
        if (Date.now() > auction.expiresAt) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUCTION_EXPIRED, 'Auction time has expired.');
        }
        if (payload.amount <= auction.currentHighestBid) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.BID_TOO_LOW, `Bid must be greater than current highest bid of $${auction.currentHighestBid}.`);
        }
        ...
        if (payload.actingPlayerId && payload.actingPlayerId !== auth.userId && !player.isBot) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUTH_FORBIDDEN, 'Cannot act on behalf of another human player.');
        }
        if (player.cash < payload.amount) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INSUFFICIENT_CASH, `Insufficient cash. You have $${player.cash}, but bid requires $${payload.amount}.`);
        }
 ...
 * 13. passAuction
 */
exports.passAuction = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, payload } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    ...
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, passerId, 'passAuction', async (t) => {
        const snap = await t.get(auctionRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUCTION_NOT_FOUND, 'Auction not found.');
        const auction = snap.data();
        if (auction.status !== 'active') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.AUCTION_NOT_ACTIVE, 'Auction not active.');
        }
        if (payload.actingPlayerId && payload.actingPlayerId !== auth.userId) {
            ...
            const actingPlayer = actingPlayerSnap.data();
        ...
        const shouldSettle = (auction.currentHighestBidderId && passed.length >= activePlayers.length - 1) ||
        ...
        if (shouldSettle) {
            ...
            if (auction.currentHighestBidderId) {
                ...
                if (winnerSnap.exists) {
                    ...
                    if (!owned.includes(auction.assetId))
                        owned.push(auction.assetId);
                    t.update(winnerRef, {
                        cash: newCash,
                        ownedSpaceIds: owned,
 ...
 * 14. resolveAuction
 */
exports.resolveAuction = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, payload } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
...
//# sourceMappingURL=auctionFunctions.js.map