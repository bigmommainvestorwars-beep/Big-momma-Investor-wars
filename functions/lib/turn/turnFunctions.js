...
exports.executeBotTurn = exports.startSpaceAuction = exports.buyProperty = exports.completeTurn = exports.executeSPAction = exports.submitMovementDecision = exports.requestRoll = void 0;
...
const boardData_1 = require("../config/boardData");
const contracts_1 = require("../types/contracts");
const db = (0, firebaseAdmin_1.getAdminFirestore)();
/**
 * 7. requestRoll
 */
exports.requestRoll = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    rateLimiter_1.RateLimiter.check(auth.userId, 'rollDice');
    const { matchId, requestId, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'requestRoll', async (t) => {
        ...
        if (match.status !== 'in_progress') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_ACTIVE, 'Match is not in progress.');
        }
        if (match.currentPlayerId !== auth.userId) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.NOT_YOUR_TURN, 'It is not your turn.');
        }
        if (match.currentPhase !== 'TURN_START') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INVALID_PHASE, `Cannot roll during phase ${match.currentPhase}.`);
        }
        const playerRef = matchRef.collection('players').doc(auth.userId);
        const playerSnap = await t.get(playerRef);
        if (!playerSnap.exists) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.PLAYER_NOT_IN_MATCH, 'Player not found in match.');
        }
        const player = playerSnap.data();
        if (player.status !== 'active') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.PLAYER_ELIMINATED, 'Player is not active.');
        }
        // Authoritative server-side roll: 1-6
        const roll = Math.floor(Math.random() * 6) + 1;
        const totalSpaces = boardData_1.TOTAL_BOARD_SPACES;
        const passedGo = player.currentSpaceIndex + roll >= totalSpaces;
        const newSpace = (player.currentSpaceIndex + roll) % totalSpaces;
        const targetSpace = (0, boardData_1.getServerSpace)(newSpace);
        if (passedGo) {
            player.cash += 200;
            player.netWorth += 200;
        }
        ...
        const newVersion = gameEngine_1.GameEngineInternal.incrementStateVersion(t, matchRef, match, expectedStateVersion);
 ...
 */
exports.submitMovementDecision = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, payload, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'submitMovementDecision', async (t) => {
        const snap = await t.get(matchRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_FOUND, 'Match not found.');
        const match = snap.data();
        if (match.currentPlayerId !== auth.userId) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.NOT_YOUR_TURN, 'Not your turn.');
        }
        const playerRef = matchRef.collection('players').doc(auth.userId);
        const playerSnap = await t.get(playerRef);
        if (!playerSnap.exists)
 ...
 * 9. executeSPAction
 */
exports.executeSPAction = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    rateLimiter_1.RateLimiter.check(auth.userId, 'spAction');
    const { matchId, requestId, payload, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'executeSPAction', async (t) => {
        const snap = await t.get(matchRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_FOUND, 'Match not found.');
        const match = snap.data();
        ...
        if (player.specialPoints < payload.spCost) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INSUFFICIENT_SP, `Insufficient SP. Requires ${payload.spCost}, available ${player.specialPoints}.`);
        }
 ...
 * 10. completeTurn
 */
exports.completeTurn = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'completeTurn', async (t) => {
        const snap = await t.get(matchRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_FOUND, 'Match not found.');
        const match = snap.data();
        ...
        const { nextPlayerId, nextTurnNumber, nextRoundNumber } = gameEngine_1.GameEngineInternal.advanceTurn(match, players);
        const newVersion = gameEngine_1.GameEngineInternal.incrementStateVersion(t, matchRef, match, expectedStateVersion);
 ...
 * 11. buyProperty
 */
exports.buyProperty = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'buyProperty', async (t) => {
        const snap = await t.get(matchRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_FOUND, 'Match not found.');
        const match = snap.data();
        ...
        const playerSnap = await t.get(playerRef);
        if (!playerSnap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.PLAYER_NOT_IN_MATCH, 'Player not in match.');
        const player = playerSnap.data();
        const targetSpace = (0, boardData_1.getServerSpace)(player.currentSpaceIndex);
        if (targetSpace.type !== 'property' && targetSpace.type !== 'company') {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INVALID_TARGET, 'Current space cannot be purchased.');
        }
        const cost = targetSpace.baseCost || 100;
        if (player.cash < cost) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INSUFFICIENT_CASH, `Insufficient cash. Cost is $${cost}, you have $${player.cash}.`);
        }
        const playersSnap = await t.get(matchRef.collection('players'));
        const allPlayers = playersSnap.docs.map((d) => d.data());
        const existingOwner = allPlayers.find((p) => p.ownedSpaceIds?.includes(targetSpace.id));
        if (existingOwner) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.ACTION_NOT_AVAILABLE, 'Property is already owned.');
        }
 ...
 */
exports.startSpaceAuction = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    const { matchId, requestId, expectedStateVersion } = request.data;
    const matchRef = db.collection('matches').doc(matchId);
    const result = await idempotencyService_1.IdempotencyService.executeWithIdempotency(matchId, requestId, auth.userId, 'startSpaceAuction', async (t) => {
        const snap = await t.get(matchRef);
        if (!snap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.MATCH_NOT_FOUND, 'Match not found.');
        const match = snap.data();
 ...
 * 13. executeBotTurn
 */
exports.executeBotTurn = (0, https_1.onCall)(async (request) => {
    const auth = auth_1.AuthGuard.assertAuthenticated(request);
    appCheck_1.AppCheckGuard.verify(request);
    ...
    const result = await db.runTransaction(async (t) => {
        ...
        if (!botId || match.currentPlayerId !== botId) {
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.NOT_YOUR_TURN, 'Not bot turn.');
        }
        const botRef = matchRef.collection('players').doc(botId);
        const botSnap = await t.get(botRef);
        if (!botSnap.exists)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.PLAYER_NOT_IN_MATCH, 'Bot not found.');
        const bot = botSnap.data();
        if (!bot.isBot)
            throw new contracts_1.ServerFunctionError(contracts_1.SERVER_ERROR_CODES.INVALID_TARGET, 'Target is not a bot.');
        // 1. Roll
        const roll = Math.floor(Math.random() * 6) + 1;
        const totalSpaces = boardData_1.TOTAL_BOARD_SPACES;
...
//# sourceMappingURL=turnFunctions.js.map